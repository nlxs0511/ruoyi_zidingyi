...
(function ($) {
    $.extend({
        ...
        table: {
            ...
            // 回显数据字典-图片
            selectDictLabelImg: function(datas, value) {
                if ($.common.isEmpty(datas) || $.common.isEmpty(value)) {
                    return '';
                }
                var actions = [];
                $.each(datas, function(index, dict) {
                    actions.push($.common.sprintf("<img onclick='bigImg()' src='%s' width='200' height='200' alt='%s' />", dict.dictValue+value+".gif", dict.dictLabel));
                });
                return actions.join('');
            },
            // 回显数据字典（字符串数组）
